...
class Window:
    def __init__(self, width=640, height=480):
        ...
        def setTxt(txt, row, col):
            ...
            button = tk.Button(
                textvariable=self.button_txt,
                bg="green",
                font=("", 20),
                command=self.changeLabel,
            )
            button.grid(
                row=2, column=0, columnspan=2, padx=5, pady=5, sticky=tk.W + tk.E
            )

            tk.Label(self.root, text=txt["c/s"], fg="red").grid(
            ...
            tk.Label(self.root, text=txt["total"], fg="red", font=("", 20)).grid(
                row=4, column=0, sticky=tk.W
            )
            self.total_txt = evalTxt(txt["none"])
            tk.Label(self.root, textvariable=self.total_txt, font=("", 20)).grid(
